...
public class Main {
    public static void main(String[] args) {
        ...
        for (Pokemon pokemon : pokemons) {
            System.out.println(pokemon.getNombre() + " - " + pokemon.getClass().getSimpleName() + " - " + pokemon.getHp() + " puntos de salud");
        }
           ...
           /* for (Pokemon pokemon : pokemons) {
            ...
            for (int i = 0; i < pokemons.length; i++) {
                ...
                if (pokemon.estaVivo()) {
                    ...
                    Pokemon p = seleccionaPokemonAtacable(pokemons, i);
                    try {
                        pokemon.atacar(p, weatherCondition);
                        System.out.println(pokemon.getNombre() + " ha atacado a " + (p).getNombre() + " - " + p.getHp() + " puntos de salud");
    ...
    private static Pokemon seleccionaPokemonAtacable(Pokemon[] pokemons, int i) {
        ...
        ArrayList<Atacar> atacables = new ArrayList<>();

        for (int j = 0; j < pokemons.length; j++) {
            //Usamos un metodo para saber si puede ser atacado o no para descartar a los divinos, vemos que no sea el mismo y que sus puntos de vida sean mayor que cero.
            // TODO: comprobar que el pokemon está vivo, que no es él mismo y que puede ser atacado
            if (pokemons[j].atacado() && pokemons[i] != pokemons[j] && pokemons[j].getHp()>0){
                System.out.println((numPokemonVivos + 1) + ": " + pokemons[j].getNombre() + " - " + pokemons[j].getClass().getSimpleName() + " - " + pokemons[j].getHp() + " puntos de salud");
                numPokemonVivos++;
                atacables.add((Atacar) pokemons[j]);
            }
        }

        int enteroIntroducido = UserDataCollector.getEnteroMinMax("Selecciona un pokemon para atacar", 1, numPokemonVivos);

        return pokemons[enteroIntroducido-1];
    }
    ...
    private static WeatherCondition generateRandomWeatherCondition(){
        ...
        return switch (random) {
            //Completé el switch agregando las distintas opciones entre 0 y 3
            //TODO: Devolver un valor de WeatherCondition
            case 0 -> WeatherCondition.LLUVIOSO;
            case 1 -> WeatherCondition.SOLEADO;
            case 2 -> WeatherCondition.TOMENTAELECTRICA;
            case 3 -> WeatherCondition.TORMENTAARENA;
            default -> null;
        };
    }
}
